...
import applaudo.pmo.Base;
import applaudo.pmo.BrowserDriver;
...
//                        webElement.click();
    ...
    public void submit() {
        ...
        List<WebElement> iframeLists = base.findElements(By.xpath("//iframe[contains(@id,'google_ads_iframe_')]"));
        for (WebElement webElem : iframeLists) {
            ((JavascriptExecutor)driver).executeScript("arguments[0].style.visibility='hidden'", webElem);
        }

        WebElement barFooter = base.findElement(By.xpath("//footer"));
        ((JavascriptExecutor)driver).executeScript("arguments[0].style.visibility='hidden'", barFooter);

        WebElement elemFix = base.findElement(By.xpath("//div[@id='fixedban']"));
        ((JavascriptExecutor)driver).executeScript("arguments[0].style.visibility='hidden'", elemFix);

        System.out.println("--- Scenario: SUBMIT TEST ---");
        WebElement btnSubmit = base.findElement(By.xpath("//button[@id='submit']"));
        base.scrollToItem(btnSubmit);
        btnSubmit.click();
